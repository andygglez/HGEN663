...
with col2:
    ...
    with st.container(border=True):
        ...
        mito.genes <- grep("^MT-", rownames(Seurat_object[["RNA"]]), value = TRUE)

        # compute proportions
        percentage_present <- function(expr_matrix, genes_vector) {
        apply(expr_matrix, 2, function(col) {
            present_genes <- rownames(expr_matrix)[col > 0]  # Get genes with expression > 0
            mean(present_genes %in% genes_vector) * 100  # Calculate percentage
        })
        }
        percent.mito <- percentage_present(DGEmatrix, mito.genes)


        # add the information back to the seurat object
        Seurat_object <- AddMetaData(Seurat_object, percent.mito, col.name = "percent.mito")

        # get a list of crystal genes (all genes starting by 'CRY') to remove crystalin contamination
        crystal.genes <- grep("^CRY[A-Z]", rownames(Seurat_object[["RNA"]]), value = TRUE)

        # compute proportions
        percent.crystal <- percentage_present(DGEmatrix, crystal.genes)

        # add the information back to the seurat object
        Seurat_object <- AddMetaData(Seurat_object, percent.crystal, col.name = "percent.crystal")
        ...
        VlnPlot(Seurat_object, features = fts, ncol = 4, pt.size = 0)
        """, language="r")
    ...
    with st.container(border=True):
        ...
        Seurat_object <-  subset(Seurat_object,  subset = nFeature_RNA > 100 & nFeature_RNA < 6000 & 
                                nCount_RNA < 10000 & nCount_RNA < 25000 &
                                percent.mito < 10 & percent.crystal < 2.5)
        dim(Seurat_object)
        """)
        
        st.markdown("Now look at the stats after filtering:")
        st.code("""
        # Get some summary stats for the sample:
    ...
    with st.container(border=True):
        ...
        st.image("images/lec8.VizDimLoadings.png")
